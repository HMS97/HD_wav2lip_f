...
class Dataset(object):
    def __init__(self, split, shared_dict):
        ...
        if len(self.all_videos) < 1000:
            self.all_videos = self.all_videos*((1000//len(self.all_videos)) + 1)    
...
def load_checkpoint(path, model, optimizer, reset_optimizer=False):
    ...
    pretrained_dict = {k: v for k, v in checkpoint["state_dict"].items() if
                       (k in model_dict) and (model_dict[k].shape == checkpoint["state_dict"][k].shape)}

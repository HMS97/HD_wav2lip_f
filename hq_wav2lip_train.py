...
class Dataset(object):
    def __init__(self, split,shared_dict):
        ...
        #https://github.com/Rudrabha/Wav2Lip/issues/50 
        #per epoch must be more than 1000 iterations
        # if len(self.all_videos) < 500:
        #     self.all_videos = self.all_videos*((500//len(self.all_videos)) + 1)    
...
def train(device, model, disc, train_data_loader, test_data_loader, optimizer, disc_optimizer,
    ...
    while global_epoch < nepochs:
        ...
        for step, (x, indiv_mels, mel, gt) in prog_bar:
            # print(step, x.shape,indiv_mels.shape,mel.shape, gt.shape)
            disc.train()
            model.train()
            ...
            # print(indiv_mels.shape,x.shape)
            g = model(indiv_mels, x)

            if hparams.syncnet_wt > 0.:
                # sync_loss = get_sync_loss(mel, F.interpolate(g, size=(5,96, 96)))
...
if __name__ == "__main__":
    ...
    manager = Manager()
    shared_dict = manager.dict()    # Dataset and Dataloader setup
